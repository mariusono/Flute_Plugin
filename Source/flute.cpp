...
flute::flute(double freq, double fs) : fs(fs), freq(freq)
{
    ...
    midiNote = 12.0 * (log2(ifqc) - log2(440.0)) + 69.0;
...
double flute::process(double fs)
{
    if (active)
    {
        ...
        kvibr = vibrato * sin(2 * double_Pi * freq_vibrato * currentSample / fs);
//        kvibr = (1-0.99) * vibrato * sin(2 * double_Pi * freq_vibrato * currentSample / fs) + 0.99 * kvibr_nm1;


        double asum1 = (ibreath * aflow1) + kenv1 + kvibr;
        
//        double pitchBendFreq = 440.0 * pow(2.0,(pitchBend_val)/12.0);
        
        double pitchBendFreq = 440.0 * pow(2.0,(midiNote + pitchBend_val - 69)/12.0);

//        DBG(String(pitchBendFreq));
//        DBG(String(ifqc));
...
void flute::updateStates(double preOutput)
{
    ...
    if (flag_stop)
    {
        ...
        rel_time = (currentSample + 100) / fs + release_time; 
